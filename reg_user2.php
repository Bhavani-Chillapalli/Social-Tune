...
if (isset($_POST['submit']))
{
	...
	if($stmt = $mysqli->prepare("insert into user_info (userid,picture,about_me,score) values ('$id','$image','$about','0')"))
		{
            $stmt->execute();
            echo "Successful" ;
		...
		else
		{
          ...
          {
	      if($stm = $mysqli->prepare("insert into images values ('$id','$image','$data')"))
		{
            $stm->execute();
            echo "Successful" ;
            header("location:reg_search.php");
		}
		else
		{
			echo "Please refill the form";
